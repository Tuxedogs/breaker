      ...
      <div className="mx-auto max-w-[1180px] space-y-4">
        ...
        <article className="base-card base-card--systems additional-panel p-4 sm:p-5">
          ...
          <div className="turret-table additional-table mt-6 overflow-hidden rounded-xl border border-cyan-300/25">
            ...
            <div className="divide-y divide-white/10">
              {velocityRows.map((row) => (
                <div key={row.weapon} className="turret-table-row additional-table-row" style={{ gridTemplateColumns: "1.4fr 1fr 1fr" }}>
                  <span className="turret-cell-action" data-label="Weapon Group">{row.weapon}</span>
                  <span className="turret-cell-keybind text-cyan-100" data-label="Velocity">{row.velocity}</span>
                  <span className="turret-cell-description text-amber-300" data-label="Effective Range">{row.effective}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="turret-table additional-table mt-5 overflow-hidden rounded-xl border border-cyan-300/25">
            <div className="turret-table-head" style={{ gridTemplateColumns: "1.6fr 1fr 1fr 1fr" }}>
              <span>Weapon</span>
              <span>Alpha</span>
              <span>DPS</span>
              <span>RPM</span>
            ...
            <div className="divide-y divide-white/10">
              {statRows.map((row) => (
                <div key={row.weapon} className="turret-table-row additional-table-row" style={{ gridTemplateColumns: "1.6fr 1fr 1fr 1fr" }}>
                  <span className="turret-cell-action" data-label="Weapon">{row.weapon}</span>
                  <span className="turret-cell-keybind" data-label="Alpha">{row.alpha}</span>
                  <span className="turret-cell-description" data-label="DPS">{row.dps}</span>
                  <span className="turret-cell-description" data-label="RPM">{row.rpm}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <article className="base-card base-card--systems additional-panel p-4 sm:p-5">
          ...
          <div className="turret-table additional-table mt-5 overflow-hidden rounded-xl border border-cyan-300/25">
            ...
            <div className="divide-y divide-white/10">
              {spreadRows.map((row) => (
                <div key={row.weapon} className="turret-table-row additional-table-row" style={{ gridTemplateColumns: "1.4fr 1fr" }}>
                  <span className="turret-cell-action" data-label="Weapon Group">{row.weapon}</span>
                  <span className="turret-cell-description text-amber-300" data-label="Spread">{row.spread}</span>
                </div>
              ))}
            </div>
          </div>
