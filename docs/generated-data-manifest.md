...
### Build Queue and fitting ownership

- Build Queue persistence and mutation use the authenticated `/api/user/build-queue` path, coordinated by `OnlinePersistenceCoordinator` and the current queue state/store integration; browser-only queue state is not a second production source of truth.
- Queue material identity is resolved through the shared logistics identity helpers. Physical inventory records are reserved by exact lot identity; duplicate recipe entries remain distinct queue instances.
- Readiness, progress, allocation, and shortage displays use the shared Build Queue logistics read models and reservation helpers. Do not introduce page-local shortage or reservation arithmetic.
- Fitting registries generated by `scripts/fitting/build_fitting_registries.py` are the canonical source for supported component statistics. Fitting-detail consumers use the shared fitting component store, with channel/build/source-type cache isolation.
- Build Queue presentation styles have one canonical ownership path: `src/components/logistics/build-queue.css`.
- Mining presentation styles have one canonical ownership path: `src/components/industry/mining/mining.css`.
- Recipe Browser presentation styles have one canonical ownership path: `src/components/industry/crafting/recipe-browser.css`.
